...
/** 消费离线端以 {@code v1:} 开头的长度前缀 HeadingPath，拒绝损坏路径而不自行猜测章节。 */
final class HeadingPathCodec {
    ...
    static List<String> decode(String encoded) {
        ...
        if (!encoded.startsWith("v1:")) throw new IllegalArgumentException("HeadingPath 编码版本非法");
        List<String> result = new ArrayList<>(); int cursor = 3;
