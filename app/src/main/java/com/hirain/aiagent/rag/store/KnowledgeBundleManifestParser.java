...
/** 仅接受共享 Manifest V2；未知字段一律拒绝，避免 Android 单侧宽松兼容造成错误激活。 */
public final class KnowledgeBundleManifestParser {
    ...
    public KnowledgeBundleManifest parse(String json) {
        ...
        if (format != KnowledgeStoreContract.FORMAT_VERSION || locator != 1) fail("MANIFEST_VERSION_UNSUPPORTED");
        ...
        if (!"DashScope".equals(text(embedding,"provider")) || !"text-embedding-v4".equals(text(embedding,"model")) || integer(embedding,"dimension") != 1024 || !"COSINE".equals(text(embedding,"distanceType")) || integer(embedding,"templateVersion") != KnowledgeStoreContract.EMBEDDING_TEMPLATE_VERSION) fail("MANIFEST_EMBEDDING_INCOMPATIBLE");
        ...
        if (integer(lexical,"analyzerVersion") != KnowledgeStoreContract.LEXICAL_FIELD_VERSION || !"BM25".equals(text(lexical,"algorithm")) || !text(parsers,"configHash").matches("sha256:[0-9a-f]{64}") || !text(chunking,"configHash").matches("sha256:[0-9a-f]{64}") || !text(corpus,"corpusHash").matches("sha256:[0-9a-f]{64}")) fail("MANIFEST_PROTOCOL_INVALID");
        Map<String,String> scopeValues=mapText(scope); Map<String,Long> counts=mapLong(object(corpus,"sourceFormatCounts",null));
        return new KnowledgeBundleManifest(format,text(root,"bundleId"),text(root,"bundleVersion"),text(root,"knowledgeScopeId"),text(root,"builderVersion"),number(root,"builtAtEpochMs"),text(root,"objectBoxVersion"),schema,locator,new KnowledgeBundleManifest.DataFile("data.mdb",number(data,"sizeBytes"),dataHash),new KnowledgeBundleManifest.Embedding("DashScope","text-embedding-v4",1024,"COSINE",KnowledgeStoreContract.EMBEDDING_TEMPLATE_VERSION),new KnowledgeBundleManifest.Hnsw(hnswHash,integer(hnsw,"neighborsPerNode"),integer(hnsw,"indexingSearchCount"),floating(hnsw,"reparationBacklinkProbability"),integer(hnsw,"vectorCacheHintSizeKB"),arrayText(hnsw,"flags")),scopeValues,text(parsers,"configHash"),formats,integer(lexical,"analyzerVersion"),text(chunking,"configHash"),new KnowledgeBundleManifest.Corpus(text(corpus,"corpusHash"),number(corpus,"documentCount"),counts,number(corpus,"parentChunkCount"),number(corpus,"childChunkCount"),number(corpus,"lexicalTermCount")));
    }
    /** V2 必须完整声明算法和预算边界；未知版本不能被 Android 静默当成 V1 激活。 */
    private static void validateChunking(JsonObject value){
        int version=integer(value,"configVersion");
        if(version==1){requireFields(value,Set.of("configVersion","configHash"));rejectUnknown(value,Set.of("configVersion","configHash"));return;}
        Set<String> fields=Set.of("configVersion","configHash","tokenEstimatorVersion","parentIdealMinTokens","parentSoftMaxTokens","parentHardMaxTokens","childIdealMinTokens","childTargetTokens","childSoftMaxTokens","childHardMaxTokens","fallbackOverlapMinRatio","fallbackOverlapMaxRatio","tableRowsPerChild","semanticStrategyVersion","paragraphReconstructionVersion","pdfColumnLayoutVersion","paragraphCosineThreshold","childMergeMaxTokens","parentSplitOverlapRatio","childForceMergeMaxTokens","childDirectMergeMaxTokens","pdfTocReferenceVersion");
        if(version!=2){fail("MANIFEST_CHUNKING_VERSION_UNSUPPORTED");} requireFields(value,fields);rejectUnknown(value,fields);
        if(!"rag-token-estimator-v2".equals(text(value,"tokenEstimatorVersion")) || integer(value,"parentIdealMinTokens")<0 || integer(value,"parentSoftMaxTokens")<integer(value,"parentIdealMinTokens") || integer(value,"parentHardMaxTokens")<integer(value,"parentSoftMaxTokens") || integer(value,"childIdealMinTokens")<0 || integer(value,"childTargetTokens")<integer(value,"childIdealMinTokens") || integer(value,"childSoftMaxTokens")<integer(value,"childTargetTokens") || integer(value,"childHardMaxTokens")<integer(value,"childSoftMaxTokens") || floating(value,"fallbackOverlapMinRatio")<0 || floating(value,"fallbackOverlapMaxRatio")<floating(value,"fallbackOverlapMinRatio") || floating(value,"fallbackOverlapMaxRatio")>0.10f || integer(value,"tableRowsPerChild")<1 || text(value,"semanticStrategyVersion").isBlank()) fail("MANIFEST_CHUNKING_INCOMPATIBLE");
    }
