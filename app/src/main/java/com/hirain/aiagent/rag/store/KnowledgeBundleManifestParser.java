...
public final class KnowledgeBundleManifestParser {
    ...
    public KnowledgeBundleManifest parse(String json) {
        ...
        if (integer(lexical,"analyzerVersion") != 1 || !"BM25".equals(text(lexical,"algorithm")) || !text(parsers,"configHash").matches("sha256:[0-9a-f]{64}") || !text(chunking,"configHash").matches("sha256:[0-9a-f]{64}") || !text(corpus,"corpusHash").matches("sha256:[0-9a-f]{64}")) fail("MANIFEST_PROTOCOL_INVALID");
    ...
    private static void validateChunking(JsonObject value){
        ...
        Set<String> fields=Set.of("configVersion","configHash","tokenEstimatorVersion","parentIdealMinTokens","parentSoftMaxTokens","parentHardMaxTokens","childIdealMinTokens","childTargetTokens","childSoftMaxTokens","childHardMaxTokens","fallbackOverlapMinRatio","fallbackOverlapMaxRatio","tableRowsPerChild","semanticStrategyVersion");
