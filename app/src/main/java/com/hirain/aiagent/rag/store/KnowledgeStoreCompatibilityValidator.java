...
public final class KnowledgeStoreCompatibilityValidator {
    ...
    public KnowledgeStoreValidationResult validate(KnowledgeBundleManifest manifest, KnowledgeStoreMetadataEntity metadata) {
        ...
        if (!equals(manifest.hnsw().configFingerprint(), metadata.hnswConfigFingerprint) || manifest.lexicalAnalyzerVersion() != metadata.lexicalAnalyzerVersion || !equals(manifest.parserConfigHash(), metadata.parserConfigHash) || !equals(manifest.chunkingConfigHash(), metadata.chunkingConfigHash) || !equals(manifest.corpus().corpusHash(), metadata.corpusHash)) return KnowledgeStoreValidationResult.failure("STORE_PROTOCOL_MISMATCH");
