...
Parent 表示一个相对完整的知识单元，Child 表示可精确召回的较小片段。
...
检索命中 Child 后，EvidenceSelector 根据预算补充：
...
Child 的 Embedding 输入必须由确定性模板生成：
...
Android 使用离线 postings 和 Child 的 `lexicalDocumentLength` 计算 BM25。
...
| Fusion Candidate | 20 |
| Final Evidence | 5 |
| Rerank 候选预算 | 独立 Token 预算，数值由服务限制和评测确定 |
| 模型侧 Evidence 预算 | 由 Context Token 预算动态分配，初始上限由评测确定 |
...
- 输入为规范化 Query 和少量融合候选；
- 候选文本必须携带必要标题路径，但不得超过服务和请求预算；
...
Parent 补全在 Rerank 之后执行，避免把大段 Parent 全量发送给 Reranker。

补全必须：

- 保留命中 Child；
- 优先补充限制、条件、Warning 和表头；
- 对同一 Parent 多个命中去重；
- 遵守 Evidence 数量和内容预算；
- 不得用 Parent 中低相关内容覆盖命中 Child；
- 保持 `SourceLocator` 和标题路径可引用。
...
内部 `RetrievalEvidence` 可以携带 Chunk ID、Dense Distance、BM25/RRF/Rerank 分数与 Rank。`diagnosticsSummary` 禁止包含异常堆栈或凭证。
