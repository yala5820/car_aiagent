...
/** 以共享 Schema 的必填字段和 V2 常量为门禁；Android 将复用同一 JSON Schema 作激活校验。 */
public final class ManifestValidator {
    public void validate(KnowledgeBundleManifest value) {
        require(value.formatVersion() == KnowledgeStoreContract.FORMAT_VERSION && value.sourceLocatorSchemaVersion() == 1, "Manifest 协议版本非法");
        ...
        require("DashScope".equals(value.embedding().get("provider")) && "text-embedding-v4".equals(value.embedding().get("model"))
                && Integer.valueOf(1024).equals(value.embedding().get("dimension"))
                && Integer.valueOf(KnowledgeStoreContract.EMBEDDING_TEMPLATE_VERSION).equals(value.embedding().get("templateVersion")), "Embedding 协议非法");
        require(Integer.valueOf(KnowledgeStoreContract.LEXICAL_FIELD_VERSION).equals(value.lexical().get("analyzerVersion")), "Lexical 字段协议非法");
