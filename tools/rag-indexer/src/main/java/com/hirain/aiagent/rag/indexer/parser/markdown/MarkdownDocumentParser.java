...
public final class MarkdownDocumentParser implements DocumentParser {
    ...
    public ParseResult parse(SourceDocument source) {
        try {
            ...
            walk(parser.parse(parseSource), source, new MarkdownHeadingPathTracker(), blocks, tables, diagnostics, new int[]{0});
    ...
    private void walk(Node node, SourceDocument source, MarkdownHeadingPathTracker headings, List<StructuredBlock> blocks, List<TableBlock> tables,
                      List<ParseDiagnostic> diagnostics, int[] ordinal) {
        for (Node child = node.getFirstChild(); child != null; child = child.getNext()) {
            if (child instanceof Heading heading) {
                String text = text(heading);
                headings.accept(heading.getLevel(), text);
                blocks.add(block(BlockType.HEADING, text, source, headings.path(), ++ordinal[0], heading));
            } else if (child instanceof Paragraph paragraph) {
                blocks.add(block(BlockType.PARAGRAPH, text(paragraph), source, headings.path(), ++ordinal[0], paragraph));
            } else if (child instanceof FencedCodeBlock code) {
                blocks.add(block(BlockType.CODE, code.getLiteral().trim(), source, headings.path(), ++ordinal[0], code));
                ...
                walk(child, source, headings, blocks, tables, diagnostics, ordinal);
            }
        }
    }
    ...
    private StructuredBlock block(BlockType type, String text, SourceDocument source, String path, int ordinal, Node node) {
        int line = node.getSourceSpans().isEmpty() ? 0 : node.getSourceSpans().get(0).getLineIndex() + 1;
        return new StructuredBlock(type, text, new SourceLocator(SourceFormat.MARKDOWN, 0, 0, null, line, line, path, ordinal), null, ExtractionConfidence.HIGH);
    }
